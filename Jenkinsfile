			        	...
			        	ENV_LOWERCASE = envname.toLowerCase()
	 					...
	 					if (RA_PATH == 'App/') {
							ENV_APPS_INFO[envname]['dir'].eachWithIndex { name, diridx ->
								...
								sh "cp -rp ${WORKSPACE}/${ETE_WORKSPACE}/${SVN_BRANCH_PATH}${ETE_TYPE}/src/${ENV_APPS_INFO[envname]['conf']} ${RA_BASE_PATH}${RA_PATH}${name}/conf; rename -- '-${ENV_LOWERCASE}' '' ${RA_BASE_PATH}${RA_PATH}${name}/conf/*"   
